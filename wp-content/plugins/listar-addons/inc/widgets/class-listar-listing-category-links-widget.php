<?php
/**
 * Widget to display a list of links to Listing Categories
 *
 ...
 * @since 1.2.9
 */
class Listar_Listing_category_Links_Widget extends WP_Widget {
	/**
	...
	public function __construct() {
		...
		$widget_title = esc_html__( 'Listing Category Links', 'listar' );

		$widget_description = array(
			'description' => esc_html__( 'Create links to listing categories', 'listar' ),
		);

		parent::__construct( 'listar_listing_category_links', '&#x27A1; LISTAR - ' . $widget_title, $widget_description );
	}
		...
		if ( ! $terms || is_wp_error( $terms ) ) :
			...
			<p>
				...
				printf(
					...
					esc_html__( 'Create listing categorys', 'listar' ),
						...
						<div class="listar-element-data">
							...
							<p>
								<select class="listar-element-data-id widefat" name="listar-element-data-id" >
									<option value="">
										<?php esc_html_e( 'Select a category', 'listar' ); ?>
									</option>
			...
			<p>
				<a href="#" class="listar-element-data-add button button-secondary">
					<?php esc_html_e( 'New category link', 'listar' ); ?>
				</a>
			</p>
					...
					if ( $terms && ! is_wp_error( $terms ) ) :
						foreach ( $terms as $term ) :
							...
							<div class="listar-widget-page-link">
								<a href="<?php echo get_term_link( $term, 'job_listing_category' ); ?>" title="<?php echo esc_attr( $term->name ); ?>"><?php echo esc_html( $term->name ); ?></a>
							</div>
							<?php
						endforeach;
					endif;
...
register_widget( 'Listar_Listing_category_Links_Widget' );
