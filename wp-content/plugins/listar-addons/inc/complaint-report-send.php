<?php
/**
 * Sends the Complaint Reports for the site administrator
 *
...
$listar_nonce = filter_input( INPUT_POST, 'listar-complaint-report-security', FILTER_CALLBACK, ['options' => 'listar_sanitize_string']);

if ( ! empty( $listar_nonce ) && wp_verify_nonce( $listar_nonce, 'listar-complaint-report-nonce' ) && 'POST' == $_SERVER['REQUEST_METHOD'] ) {

	// Get the form fields and remove whitespace.
	$custom_complaint_email = sanitize_email( wp_unslash( trim( get_option( 'listar_complaint_reports_for' ) ) ) );
	$admin_email = ! empty( $custom_complaint_email ) ? $custom_complaint_email : get_bloginfo( 'admin_email' );
	$listar_sender_name = isset( $_POST['listar_complaint_sender_name'] ) ? sanitize_text_field( wp_unslash( strip_tags( trim( $_POST['listar_complaint_sender_name'] ) ) ) ) : '';
	$listar_sender_name = str_replace( array( "\r", "\n" ), '', $listar_sender_name );
	$listar_reply_to_name = $listar_sender_name;
	$listar_sender_email = isset( $_POST['listar_complaint_sender_email'] ) ? sanitize_email( wp_unslash( trim( $_POST['listar_complaint_sender_email'] ) ) ) : '';
	$listar_listing_owner_name = isset( $_POST['listar_complaint_listing_owner_name'] ) ? sanitize_text_field( wp_unslash( strip_tags( trim( $_POST['listar_complaint_listing_owner_name'] ) ) ) ) : '';
	$listar_listing_owner_name = str_replace( array( "\r", "\n" ), '', $listar_listing_owner_name );
	$listar_listing_owner_email = isset( $_POST['listar_complaint_listing_owner_email'] ) ? sanitize_email( wp_unslash( trim( $_POST['listar_complaint_listing_owner_email'] ) ) ) : '';
	$listar_site_name = isset( $_POST['listar_complaint_site_name'] ) ? sanitize_text_field( wp_unslash( strip_tags( trim( $_POST['listar_complaint_site_name'] ) ) ) ) : '';
	$listar_listing_title = isset( $_POST['listar_complaint_listing_title'] ) ? sanitize_text_field( wp_unslash( strip_tags( trim( $_POST['listar_complaint_listing_title'] ) ) ) ) : '';
	$listar_current_page_link = isset( $_POST['listar_complaint_current_page_link'] ) ? esc_url( wp_unslash( strip_tags( trim( $_POST['listar_complaint_current_page_link'] ) ) ) ) : '';
	$listar_message_subject = isset( $_POST['listar_complaint_report_subject'] ) ? sanitize_text_field( wp_unslash( strip_tags( trim( $_POST['listar_complaint_report_subject'] ) ) ) ) : '';
	$listar_sender_message = isset( $_POST['listar_complaint_sender_message'] ) ? wp_kses( wp_unslash( trim( $_POST['listar_complaint_sender_message'] ) ), 'post' ) : '';
	$listar_message_content = isset( $_POST['listar_complaint_report_template'] ) ? wp_kses( wp_unslash( trim( $_POST['listar_complaint_report_template'] ) ), 'post' ) : '';
	...
	if ( empty( $listar_sender_name ) ) {
		$listar_sender_name = esc_html__( 'Anonymous visitor', 'listar' );
		$listar_reply_to_name = $listar_listing_owner_name;
	}
	...
	if ( empty( $listar_sender_name ) || empty( $listar_listing_owner_name ) || empty( $listar_sender_message ) || ! filter_var( $listar_listing_owner_email, FILTER_VALIDATE_EMAIL ) ) {
	...
	$listar_message_recipient = $admin_email;
	
	// Set the reply email address.
	$listar_message_reply = ! empty( $listar_sender_email ) ? $listar_sender_email : $listar_listing_owner_email;
	
	if ( empty( $listar_sender_email ) ) {
		$listar_sender_email = esc_html__( 'Not required', 'listar' );
	}
	...
	$listar_message_content = str_replace( '[listar_site_name]', $listar_message_from_name, $listar_message_content );
	$listar_message_content = str_replace( '[listar_listing_owner_name]', $listar_listing_owner_name, $listar_message_content );
	$listar_message_content = str_replace( '[listar_listing_owner_email]', $listar_listing_owner_email, $listar_message_content );
	...
	// Don't procceed if "From" email wasn't set on Easy WP SMTP.
	...
	$listar_email_headers .= "Reply-To: $listar_reply_to_name < $listar_message_reply >\r\n";
