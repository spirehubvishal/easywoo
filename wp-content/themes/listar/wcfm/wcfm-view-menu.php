<?php
global $wp, $WCFM, $WCFM_Query, $wpdb, $blog_id;
...
if ( $is_vendor && false !== strpos( $store_name, '</a></a>' ) ) {
	$temp = str_replace( '</a></a>', '', $store_name );
	$temp2 = explode( '><', $temp );
	$temp3 = explode( '>', $temp2[1] );
	$store_name = $temp2[0] . '>' . $temp3[1] . '</a>';
}
...
?>
<div id="wcfm_menu" <?php if ( $toggle_state && ( $toggle_state == 'yes' ) ) {
	echo 'class="wcfm_menu_toggle"';
} ?>>
...
<?php if ( apply_filters( 'wcfm_is_allow_home_in_menu', true ) ) { ?>
	...
	if ( ! empty( $wcfm_formeted_menus ) ) {
		foreach ( $wcfm_formeted_menus as $wcfm_menu_key => $wcfm_menu_data ) {
			$wcfm_menu_for = 'both';
			if ( isset( $wcfm_menu_data[ 'menu_for' ] ) )
				$wcfm_menu_for = $wcfm_menu_data[ 'menu_for' ];
			if ( ( $wcfm_menu_for == 'admin' ) && wcfm_is_vendor() )
				continue;
			if ( ( $wcfm_menu_for == 'vendor' ) && ! wcfm_is_vendor() )
				continue;
			...
			if ( ! empty( $wcfm_menu_data[ 'label' ] ) && ! empty( $wcfm_menu_data[ 'url' ] ) ) {
				if ( ! isset( $wcfm_menu_data[ 'capability' ] ) || empty( $wcfm_menu_data[ 'capability' ] ) || apply_filters( $wcfm_menu_data[ 'capability' ], true ) ) {
					$is_active = false;
					if ( isset( $wp->query_vars[ $wcfm_menu_key ] ) )
						$is_active = true;
					if ( ! $is_active && $current_endpoint && isset( $menu_active_dependent_list[ $current_endpoint ] ) && ( $menu_active_dependent_list[ $current_endpoint ] == $wcfm_menu_key ) )
						$is_active = true;
					?>
				...
				?>
						...
						<?php if ( apply_filters( 'wcfm_is_pref_hover_submenu', true ) ) { ?>
							<?php if ( ! isset( $wcfm_menu_data[ 'submenu_capability' ] ) || empty( $wcfm_menu_data[ 'submenu_capability' ] ) || apply_filters( $wcfm_menu_data[ 'submenu_capability' ], true ) ) { ?>
								...
								<?php if ( isset( $wcfm_menu_data[ 'new_label' ] ) && ! empty( $wcfm_menu_data[ 'new_label' ] ) ) {
									_e( $wcfm_menu_data[ 'new_label' ], 'wc-frontend-manager' );
								} else {
									_e( 'Add New', 'wc-frontend-manager' );
								} ?>
										</a>
									</span>
						<?php } ?>
