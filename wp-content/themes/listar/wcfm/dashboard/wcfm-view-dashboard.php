	...
	<div class="wcfm-collapse-content">
		...
		<?php if( apply_filters( 'wcfm_is_pref_stats_box', true ) ) { ?>
			<div class="wcfm_dashboard_stats">
					...
					<?php if( apply_filters( 'wcfm_is_allow_stats_block_sold_item', true ) ) { ?>
						<div class="wcfm_dashboard_stats_block">
							<a href="<?php echo apply_filters( 'sales_by_product_report_url', get_wcfm_reports_url( ), '' ); ?>">
								<span class="wcfmfa fa-store-alt"></span>
								<div>
					...
					<?php if( apply_filters( 'wcfm_is_allow_view_commission', true ) && apply_filters( 'wcfm_is_allow_stats_block_commission', true ) ) { ?>
						<div class="wcfm_dashboard_stats_block">
							<a href="<?php echo get_wcfm_reports_url( ); ?>">
								<span class="wcfmfa fa-money fa-coins"></span>
								<div>
				...
				<?php } ?>
				<?php if( listar_wp_job_manager_active() ) {
					$listar_job_dashboard_url = job_manager_get_permalink( 'job_dashboard' ); 
					$count_listings_published = $wpdb->get_var( "SELECT COUNT(ID) FROM {$wpdb->posts} WHERE post_type = 'job_listing' and post_status = 'publish'" );
					$count_listings_pending = $wpdb->get_var( "SELECT COUNT(ID) FROM {$wpdb->posts} WHERE post_type = 'job_listing' and post_status = 'pending'" );
					
					if ( empty( $listar_job_dashboard_url ) ) {
						$listar_job_dashboard_url = admin_url( 'edit.php?post_type=job_listing' );
					}
					...
					$count_published = absint( $wpdb->get_var( $sql ) ); // WPCS: unprepared SQL ok.
										
					/* Count pending products */

					$sql  = "SELECT COUNT( DISTINCT {$wpdb->posts}.ID ) FROM {$wpdb->posts} ";
					$sql .= $tax_query_sql['join'];
					$sql .= " WHERE {$wpdb->posts}.post_type = 'product' AND {$wpdb->posts}.post_status = 'pending' ";
					$sql .= $tax_query_sql['where'];

					$count_pending = absint( $wpdb->get_var( $sql ) ); // WPCS: unprepared SQL ok.
					
					$listar_product_dashboard_url = get_wcfm_products_url();
					$count_products_published = $count_published;
					$count_products_pending = $count_pending;
					
					$listar_post_dashboard_url = get_wcfm_articles_url();
					$count_posts_published = $wpdb->get_var( "SELECT COUNT(ID) FROM {$wpdb->posts} WHERE post_type = 'post' and post_status = 'publish'" );
					$count_posts_pending = $wpdb->get_var( "SELECT COUNT(ID) FROM {$wpdb->posts} WHERE post_type = 'post' and post_status = 'pending'" );
					?>
					<div class="wcfm_dashboard_stats_block listar-dashboard-post-count">
						<a href="<?php echo esc_url( $listar_job_dashboard_url ); ?>" target="_blank">
							<span class="wcfmfa fa-map-marker-alt"></span>
							<div>
								<div class="listar-dashboard-pending">
									<?php printf( _n( "<strong>%s pending</strong>", "<strong>%s pending</strong>", $count_listings_pending, 'listar' ), $count_listings_pending ); ?>
								</div>
								<?php printf( _n( "<strong>%s listing</strong>", "<strong>%s listings</strong>", $count_listings_published, 'listar' ), $count_listings_published ); ?>
								<br /><?php _e( 'published', 'listar' ); ?>
							</div>
						</a>
					</div>
					<?php					
					if ( listar_third_party_reviews_active() || listar_built_in_reviews_active() ) :						
						$count_comments_published = $wpdb->get_var(
							...
							AND cmeta.meta_key = "review_stars"'
						);

						$count_comments_pending = $wpdb->get_var(
							'SELECT COUNT( comments.comment_ID ) FROM '. $wpdb->comments .' as comments
							LEFT JOIN '.$wpdb->commentmeta.' AS cmeta ON comments.comment_ID = cmeta.comment_id
						...
						?>
						<div class="wcfm_dashboard_stats_block listar-dashboard-post-count">
							<a href="<?php echo admin_url( 'edit-comments.php' ); ?>" target="_blank">
								<span class="wcfmfa fa-star"></span>
								<div>
									<div class="listar-dashboard-pending">
										<?php printf( _n( "<strong>%s pending</strong>", "<strong>%s pending</strong>", $count_comments_pending, 'listar' ), $count_comments_pending ); ?>
									</div>
									<?php printf( _n( "<strong>%s listing review</strong>", "<strong>%s listing reviews</strong>", $count_comments_published, 'listar' ), $count_comments_published ); ?>
									<br /><?php _e( 'published', 'listar' ); ?>
								</div>
							</a>
						</div>
					...
					?>
					<div class="wcfm_dashboard_stats_block listar-dashboard-post-count">
						<a href="<?php echo esc_url( $listar_product_dashboard_url ); ?>">
							<span class="wcfmfa fa-cube"></span>
							<div>
								<div class="listar-dashboard-pending">
									<?php printf( _n( "<strong>%s pending</strong>", "<strong>%s pending</strong>", $count_products_pending, 'listar' ), $count_products_pending ); ?>
								</div>
								<?php printf( _n( "<strong>%s product</strong>", "<strong>%s products</strong>", $count_products_published, 'listar' ), $count_products_published ); ?>
								<br /><?php _e( 'published', 'listar' ); ?>
							</div>
						</a>
					</div>
					<div class="wcfm_dashboard_stats_block listar-dashboard-post-count">
						<a href="<?php echo esc_url( $listar_post_dashboard_url ); ?>">
							<span class="wcfmfa fa-file-alt"></span>
							<div>
								<div class="listar-dashboard-pending">
									<?php printf( _n( "<strong>%s pending</strong>", "<strong>%s pending</strong>", $count_posts_pending, 'listar' ), $count_posts_pending ); ?>
								</div>
								<?php printf( _n( "<strong>%s articles</strong>", "<strong>%s articles</strong>", $count_posts_published, 'listar' ), $count_posts_published ); ?>
								<br /><?php _e( 'published', 'listar' ); ?>
							</div>
						</a>
					</div>
