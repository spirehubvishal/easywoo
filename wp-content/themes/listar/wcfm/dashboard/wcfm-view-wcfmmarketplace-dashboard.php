		...
		<?php if( apply_filters( 'wcfm_is_pref_stats_box', true ) ) { ?>
			<div class="wcfm_dashboard_stats">
				...
				<?php if( listar_wp_job_manager_active() ) {
					...
					$count_listings_published = $wpdb->get_var( "SELECT COUNT(ID) FROM {$wpdb->posts} WHERE post_type = 'job_listing' AND post_status = 'publish' AND post_author = '" . $user_id . "'" );
					$count_listings_pending = $wpdb->get_var( "SELECT COUNT(ID) FROM {$wpdb->posts} WHERE post_type = 'job_listing' AND post_status = 'pending' AND post_author = '" . $user_id . "'" );
					...
					$sql  = "SELECT COUNT( DISTINCT {$wpdb->posts}.ID ) FROM {$wpdb->posts} ";
					$sql .= $tax_query_sql['join'];
					$sql .= " WHERE {$wpdb->posts}.post_type = 'product' AND {$wpdb->posts}.post_status = 'pending' AND post_author = '" . $user_id . "'";
					$sql .= $tax_query_sql['where'];

					$count_products_pending  = absint( $wpdb->get_var( $sql ) ); // WPCS: unprepared SQL ok.

					$listar_post_dashboard_url = get_wcfm_articles_url();
					$count_posts_published = $wpdb->get_var( "SELECT COUNT(ID) FROM {$wpdb->posts} WHERE post_type = 'post' and post_status = 'publish' AND post_author = '" . $user_id . "'" );
					$count_posts_pending = $wpdb->get_var( "SELECT COUNT(ID) FROM {$wpdb->posts} WHERE post_type = 'post' and post_status = 'pending' AND post_author = '" . $user_id . "'" );
					?>
					...
					if ( listar_third_party_reviews_active() || listar_built_in_reviews_active() ) :						
						...
						?>
						<div class="wcfm_dashboard_stats_block listar-dashboard-post-count">
							<a href="#" class="listar-dashboard-reviews-block listar-dashboard-reviews-block-vendor">
