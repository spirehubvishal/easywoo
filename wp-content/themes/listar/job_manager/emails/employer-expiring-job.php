...
if ( $expiring_today ) {
	echo wp_kses_post(
		sprintf(
			...
			__( 'The following listing is expiring today from <a href="%s">%s</a>.', 'listar' ),
			home_url(),
			get_bloginfo( 'name' )
		)
	);
	...
	echo wp_kses_post(
		sprintf(
			// translators: %1$s placeholder is URL to the blog. %2$s placeholder is the name of the site.
			__( 'The following listing is expiring soon from <a href="%s">%s</a>.', 'listar' ),
			home_url(),
			get_bloginfo( 'name' )
		)
	);
...
echo wp_kses_post(
	sprintf(
		...
		' ' . __( 'Visit the <a href="%s">listing dashboard</a> to manage the listing.', 'listar' ),
