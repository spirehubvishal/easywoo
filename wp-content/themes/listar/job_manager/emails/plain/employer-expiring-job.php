<?php
/**
 * Email content when notifying employers of an expiring listing.
 *
...
if ( $expiring_today ) {
	...
	printf( esc_html__( 'The following listing is expiring today from %1$s (%2$s).', 'listar' ), esc_html( get_bloginfo( 'name' ) ), esc_url( home_url() ) );
} else {
	// translators: %1$s placeholder is the name of the site, %2$s placeholder is URL to the blog.
	printf( esc_html__( 'The following listing is expiring soon from %1$s (%2$s).', 'listar' ), esc_html( get_bloginfo( 'name' ) ), esc_url( home_url() ) );
}
// translators: Placeholder %s is the listing dashboard URL.
printf( ' ' . esc_html__( 'Visit the listing dashboard (%s) to manage the listing.', 'listar' ), esc_url( job_manager_get_permalink( 'job_dashboard' ) ) );
