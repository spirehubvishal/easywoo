...
if ( $expiring_today ) {
	printf(
		...
		esc_html__( 'The following listing is expiring today from %1$s (%2$s).', 'listar' ),
	...
	printf(
		// translators: %1$s placeholder is the name of the site, %2$s placeholder is URL to the blog.
		esc_html__( 'The following listing is expiring soon from %1$s (%2$s).', 'listar' ),
		esc_html( get_bloginfo( 'name' ) ),
		esc_url( home_url() )
	);
}
...
printf(
	...
	' ' . esc_html__( 'Visit WordPress admin (%s) to manage the listing.', 'listar' ),
