<?php
/**
 * Email content when notifying admin of a new listing.
 *
...
printf( esc_html__( 'A new listing has been submitted to %1$s (%2$s).', 'listar' ), esc_html( get_bloginfo( 'name' ) ), esc_url( home_url() ) );
switch ( $job->post_status ) {
	case 'publish':
		printf( ' ' . esc_html__( 'It has been published and is now available to the public.', 'listar' ) );
		break;
	case 'pending':
		// translators: Placeholder %s is the admin listings URL.
		printf( ' ' . esc_html__( 'It is awaiting approval by an administrator in WordPress admin (%s).', 'listar' ), esc_url( admin_url( 'edit.php?post_type=job_listing' ) ) );
		break;
}
