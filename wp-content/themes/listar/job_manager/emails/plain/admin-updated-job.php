...
printf( esc_html__( 'A listing has been updated on %1$s (%2$s).', 'listar' ), esc_html( get_bloginfo( 'name' ) ), esc_url( home_url() ) );
switch ( $job->post_status ) {
	...
	case 'pending':
		...
		printf( ' ' . esc_html__( 'The listing is not publicly available until the changes are approved by an administrator in the site\'s WordPress admin (%s).', 'listar' ), esc_url( admin_url( 'edit.php?post_type=job_listing' ) ) );
		break;
}
