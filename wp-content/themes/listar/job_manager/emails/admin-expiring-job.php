<?php
/**
 * Email content when notifying the administrator of an expiring listing.
 *
...
if ( $expiring_today ) {
	...
	echo wp_kses_post( sprintf( __( 'The following listing is expiring today from <a href="%1$s">%2$s</a>.', 'listar' ), esc_url( home_url() ), esc_html( get_bloginfo( 'name' ) ) ) );
} else {
	// translators: %1$s placeholder is URL to the blog. %2$s placeholder is the name of the site.
	echo wp_kses_post( sprintf( __( 'The following listing is expiring soon from <a href="%1$s">%2$s</a>.', 'listar' ), esc_url( home_url() ), esc_html( get_bloginfo( 'name' ) ) ) );
}

echo ' ';

// translators: Placeholder is URL to site's WP admin.
echo wp_kses_post( sprintf( __( 'Visit <a href="%s">WordPress admin</a> to manage the listing.', 'listar' ), esc_url( $edit_post_link ) ) );
echo '</p>';
