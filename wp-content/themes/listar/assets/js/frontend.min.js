...
/* global AOS, listarMinMapZoomLevel, listarMaxMapZoomLevel, listarThemeURL, L, listarLocalizeAndAjax, listarAjaxPostsParams, listarMapMarkers, listarSiteCountryCode, jQuery, vhCheck, ifvisible, wpMyInstagramVars, listarSiteURL, opr, safari, InstallTrigger, MSStream, tinyMCE */
...
( function ( $ ) {
	...
	function preloadImages(array, waitForOtherResources, timeout) {
		...
		if (!waitForOtherResources || document.readyState === 'complete') {
		    ...
		    /* linereport += '---' + new Error().stack.split( 'ver=1.4.6:' )[1].split(':')[0] + '---, '; */
					...
					dlTags.each( function () {
						if ( ! $( this )[0].hasAttribute( 'data-skip-tag' ) ) {
							if ( isInViewport( $( this ) ) ) {
								...
								/* linereport += '---' + new Error().stack.split( 'ver=1.4.6:' )[1].split(':')[0] + '---, '; */ setTimeout( function () {
			...
			setTimeout( function () {
				...
				if ( theBody.hasClass( 'single-post' ) ) {
					if ( $( '.listar-single-content p.has-background[class*="fa fa-"], .listar-single-content p.has-background[class*="icon-"]' ).length ) {
						iconClass = $( '.listar-single-content p.has-background[class*="fa fa-"], .listar-single-content p.has-background[class*="icon-"]' ).eq( 0 ).attr( 'class' );
					}
	...
	function createPriceListItem( listItemID, listCategoryID, priceTag, priceTitle, pricePrice, priceDescr ) {
...
/* linereport += '---' + new Error().stack.split( 'ver=1.4.6:' )[1].split(':')[0] + '---, '; */ 
	...
	function startAfterFirstInteraction() {
		...
		/* linereport += '---' + new Error().stack.split( 'ver=1.4.6:' )[1].split(':')[0] + '---, '; */ setInterval( function () {
			var scrollTop = theDocument.scrollTop();
			...
			if ( theBody.hasClass( 'listar-ajax-pagination' ) && theBody.hasClass( 'listar-ajax-infinite-scroll' ) ) {
				var loadMore = $( '.listar-main-block .listar-more-results' );
		...
		if ( 'undefined' !== typeof listarMapMarkers ) {
			...
			for ( var m = 0; m < markersCount; m++ ) {
				...
				if ( $( '.listar-map ~ .listar-aside-list' ).length ) {
					...
					$( '.listar-temp-div' ).addClass( tempListingItem.id );

					if ( 'undefined' !== typeof tempListingItem.listingID ) {
						$( '.listar-temp-div' ).attr( 'id', 'listar-listing-map-' + tempListingItem.listingID );
						lastListingID = tempListingItem.listingID;
					}

					if ( 'undefined' !== typeof tempListingItem.link ) {
						$( '.listar-temp-div' ).append( '<a href="' + tempListingItem.link + '"></a>' );
					} else {
						$( '.listar-temp-div' ).append( '<a href="#"></a>' );
					}

					if ( - 1 !== icon.indexOf( '<svg' ) ) {
						$( '.listar-temp-div a' ).append( '<div class="listar-aside-post-icon">' + icon + '</div>' );
					} else {
						$( '.listar-temp-div a' ).append( '<div class="listar-aside-post-icon ' + icon + '"></div>' );
					}

					if ( 'undefined' !== typeof tempListingItem.categoryColor ) {
						$( '.listar-temp-div .listar-aside-post-icon' ).css( 'background-color', categoryColor );
					}

					if ( isMobile() ) {
						$( '.listar-temp-div a' ).append( '<div class="listar-show-map-popup"></div>' );
					}

					$( '.listar-temp-div a' ).append( '<div class="listar-single-listing-link"></div>' );

					if ( 'undefined' !== typeof tempListingItem.featText ) {
						$( '.listar-temp-div a' ).append( '<div class="listar-ribbon">' + tempListingItem.featText + '</div>' );
					}

					if ( 'undefined' !== typeof tempListingItem.rating ) {
						$( '.listar-temp-div a' ).append( '<div class="listar-listing-rating">' + tempListingItem.rating + '</div>' );
					}

					if ( 'undefined' !== typeof tempListingItem.trending ) {
						$( '.listar-temp-div' ).attr( 'data-has', 'trending' );
						$( '.listar-temp-div a' ).append( '<div class="listar-trending-icon fal fa-bolt"></div>' );
					}

					if ( 'undefined' !== typeof tempListingItem.claimed ) {
						$( '.listar-temp-div' ).attr( 'data-has', 'claim' );
						$( '.listar-temp-div a' ).append( '<div class="listar-claimed-icon"></div>' );
					}

					if ( 'undefined' === typeof tempListingItem.lat || 'undefined' === typeof tempListingItem.lng ) {
						$( '.listar-temp-div' ).attr( 'data-no-coordinates', 'no-coordinates' );
						$( '.listar-temp-div a' ).append( '<div class="listar-no-map-nav icon-map-marker-crossed"></div>' );

						if ( $( '.listar-temp-div .icon-map-marker' ).length ) {
							$( '.listar-temp-div .icon-map-marker' ).prop( 'outerHTML', '' );
						}
					}
					...
					if ( 'undefined' !== typeof tempListingItem.img ) {
						if ( -1 === tempListingItem.img.indexOf( '5BAEAAAAALAAAAAABAAEAAAICRAEAOw' ) ) {
							...
							$( '.listar-temp-div .listar-aside-post-pic' ).append( listingImage );
							...
							if ( 'undefined' !== typeof tempListingItem.logo ) {
								if ( -1 === tempListingItem.logo.indexOf( '5BAEAAAAALAAAAAABAAEAAAICRAEAOw' ) ) {
									...
									$( '.listar-temp-div a' ).append( listingLogo );
									$( '#listar-listing-logo-' + tempListingItem.id ).css( { 'background-image' : 'url(' + tempListingItem.logo + ')' } );
								}
							}
						} else {
							$( '.listar-temp-div' ).attr( 'data-no-image', 'listar-no-image' );
						}
					} else {
						$( '.listar-temp-div' ).attr( 'data-no-image', 'listar-no-image' );
					}

					$( '.listar-temp-div .listar-aside-post-data' ).append( '<div class="listar-aside-post-title"></div>' );

					if ( 'undefined' !== typeof tempListingItem.title ) {
						$( '.listar-temp-div .listar-aside-post-title' ).append( tempListingItem.title );
					}

					if ( 'undefined' !== typeof tempListingItem.category ) {
						$( '.listar-temp-div .listar-aside-post-data' ).append( '<div class="listar-aside-post-category"></div>' );
						$( '.listar-temp-div .listar-aside-post-category' ).append( tempListingItem.category );
					}

					if ( 'undefined' !== typeof tempListingItem.region ) {
						$( '.listar-temp-div .listar-aside-post-data' ).append( '<div class="listar-aside-post-region"></div>' );
						$( '.listar-temp-div .listar-aside-post-region' ).append( tempListingItem.region );
					}
	...
	function leafletMapInit() {
		if ( false === mapStarted ) {
			...
			if ( 'undefined' !== typeof listarMapMarkers && $( '#map' ).length ) {
				...
				map = new L.Map( 'map', { center: latlng, zoom: 14, dragging: dragMap, tap: tapMap, layers: [ osm ] } );
	...
	function listarExecute() {
				...
				$( '.fieldset-job_business_price_list_content' ).each( function () {
					...
					if ( false !== parsedData ) {
						...
						$.each( parsedData, function() {
							...
							createPriceListItem( priceItemID, priceCatID, priceTag, priceTitle, pricePrice, priceDescr );
		...
		if ( $.isFunction( $.fn.select2 ) ) {				
			...
			setTimeout( function () {
				$( 'select[name*="job_business_products_list"],select[name*="job_business_booking_method"]' ).each( function () {
					if ( $( this ).siblings( '.select2' ).length ) {
						$( this ).siblings( '.select2' ).prop( 'outerHTML', '' );
						$( this ).select2( 'destroy' );
						$( this ).select2();
	...
	/* linereport += '---' + new Error().stack.split( 'ver=1.4.6:' )[1].split(':')[0] + '---, '; */
		...
		$( 'input[name=submit_job]' ).on( 'click', function ( e ) {
			...
			$( '#job_business_raw_contents' ).each( function () {
				...
				$( '[id*="job_business_document_"][id*="_title"], .listar-price-builder-category-val, .listar-price-item-title-val, .listar-price-item-descr-val, #job_listing_subtitle, #job_tagline' ).each( function () {
					...
					if ( ! priceListCategories.length ) {
						$( '.listar-price-item' ).each( function () {
							...
							if ( '' !== priceTitle ) {
								...
								if ( '' !== priceTitle ) {
									JSONValues += '{"category_id":"' + priceCatID + '","category":"' + priceCat + '","item_id":"' + priceItemID + '","tag":"' + priceTag + '","title":"' + priceTitle + '","price":"' + pricePrice + '","description":"' + priceDescr + '"},';
								} else {
									if ( ! hasIssues ) {
										hasIssues = true;
...
/* linereport += '---' + new Error().stack.split( 'ver=1.4.6:' )[1].split(':')[0] + '---, '; */
		...
		var tokens = [
			'6yJDWO56FlhU8AGQLiqLn1XFcu9SUf1s4k81RAzZq7HoBlfoaHAwe0jov4sUk4jj',
			'YPRMHgjAiNtBhUWczIRAaCujwYXhAukJL9tXDMn3z2DrLbiPHsVlYrIwfdfnDKjP',
			'YjD22JtpHsOdODh6mskQ8eSjXPdPWvTIrWAxpE3LRjUgrAFSm9YbRKT6R97kZVEN',
			'sJhFjwEk1eq6laVO5iOP5OS7NgB8ItsBytqxMzs5ViKMRMHeHl9J4LdU4PrEUupn',
			'o3X2KS8XuBzpdk7yZTqGholHUk3JTor72q51FQmzSFzLS3UIe2gQVkCKTh9VT0bm'
		];
		
		/* From zero (included) until 4 (included) */
		var tokenMap = tokens[ Math.floor( Math.random() * 5 ) ];

		osmUrl = urlProtocol + '://tile.jawg.io/sunny/{z}/{x}/{y}.png?access-token=' + tokenMap;
		...
		if ( 'mapbox' === listarLocalizeAndAjax.listarMapTilesProvider ) {
			var mapboxToken = listarLocalizeAndAjax.listarMapboxToken;
