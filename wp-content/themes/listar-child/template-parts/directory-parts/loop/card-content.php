<?php
/**
 * Template part to output listing card content.
 *
...
$listar_listing_title              = listar_excerpt_limit( get_the_title( $listar_current_post_id ), 80, false, '', false );
...
if ( ! empty( $listar_lat ) && ! empty( $listar_lng ) && listar_addons_active() ) {
...
.listar-circular-wrapper .listar-card-content-image{
	border-radius: 16px !important;
...
.custom-listar-listing-rating::before {
	content: '\f005';
...
.listar-circular-wrapper .listar-listing-logo-wrapper{
	right:auto;
	bottom:-25px;
	left:20px;
...
.listar-circular-wrapper .listar-listing-logo-wrapper .listar-listing-logo {
    width: 75px;
	height:75px;
}
